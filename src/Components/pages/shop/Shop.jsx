...
const Shop = () => {
  ...
  // Initialize categories with categoryID from URL if exists
  ...
  const handleCheckboxChange = (e, state, setState) => {
    ...
    value = !isNaN(value) ? Number(value) : value; // keep numbers as numbers
    if (e.target.checked) {
    ...
    const fetchingData = async () => {
      try {
        ...
        const response = await axios.post(
          `${serverUrl}/api/product/filter_product_with_pagination?page=${page}&sort=${
            sort || ""
          }&search=${search || ""}`,
          {
            categories,
  ...
  }, [categories, types, ratings, prices, page, sort, search]);
            ...
            <Collapse isOpened={openType === category._id}>
              <div className="border border-gray-300 px-2 w-full">
                {category.children.map((type) => (
                  <span
                    key={type._id}
                    className="flex items-center cursor-pointer hover:text-orange-500 justify-between my-1 py-1 text-[10px]"
                  >
                    <input
                      type="checkbox"
                      ...
                      value={type._id}
                      className="border"
                      onChange={(e) => handleCheckboxChange(e, types, setTypes)}
                    />
    ...
    <div>
      ...
      <div className="flex bg-zinc-50 h-10 w-full items-center border border-green-50">
        ...
        <div className="flex justify-between flex-1 h-full items-center font-bold text-xs">
          {/* View Toggle */}
          <div className="flex justify-between h-full items-center gap-1 pl-2">
            <button
              className={
                viewProducts === "grid" ? "text-gray-900" : "text-gray-500"
              }
              onClick={() => setViewProducts("grid")}
            >
              <IoGrid className="text-lg md:text-[20px]" />
            </button>
            <button
              className={
                viewProducts === "list" ? "text-gray-900" : "text-gray-500"
              }
              onClick={() => setViewProducts("list")}
            >
              <FaThList className="text-lg md:text-[20px]" />
            </button>
          </div>
        ...
        <div className="relative h-full">
          ...
          >
            ...
            <div className="border border-gray-300 my-1 h-auto bg-zinc-50 px-1">
              ...
              {/* Price Filter */}
              <div className="py-3">
                <h1 className="font-bold text-xs px-2 pt-1">Filter by Price</h1>
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="flex justify-center">
                <Pagination
                  count={productData?.total_pages || 0}
                  page={page}
                  variant="outlined"
